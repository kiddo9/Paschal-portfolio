...
function Index() {
  ...
  function selectPage(menuName) {
    if (change === menuName) return; // Don't animate if clicking the same page

    // Start exit animation
    setAnimating(true);
    setExitingPage(change);

    // After exit animation completes, switch pages
    setTimeout(() => {
      setchange(menuName);

      // Reset animation states after a brief delay
      setTimeout(() => {
        setAnimating(false);
        setExitingPage(null);
      }, 50);
  ...
  // Animation classes
  const pageExitClass = "animate-fadeOut opacity-0 transform translate-x-full";
  const pageEnterClass = "animate-fadeIn opacity-100 transform translate-x-0";
  ...
  function handleEmailClick(email, name, subject, body) {
    if (!email || !name || !subject || !body) {
      alert("Please fill in all fields before sending an email.");
      return;
    }
    const Email = email;
  ...
  return (
    <div className="h-full w-full bg-[#222222] fixed">
